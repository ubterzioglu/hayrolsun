  ...
  return (
    <footer className={`py-8 border-t transition-all duration-300 ${darkMode ? 'border-slate-800/50 bg-gradient-to-r from-slate-900 to-gray-900' : 'border-emerald-200/50 bg-gradient-to-r from-white to-emerald-50/50'}`}>
      <div className="container mx-auto px-4 text-center">
        ...
        <p className={`mb-3 text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          Rüya tabirleri Kur'an-ı Kerim ve hadis-i şeriflere dayanmaktadır.
        </p>
        <div className={`flex flex-wrap justify-center gap-3 mb-3 ${darkMode ? 'text-gray-500' : 'text-gray-500'}`} style={{ fontSize: '0.7rem' }}>
        ...
        <p className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-500'}`} style={{ fontSize: '0.7rem' }}>
