...
module.exports = async function handler(req, res) {
  ...
  try {
    const sql =
      vote === 'like'
        ? 'UPDATE articles SET likes = likes + 1 WHERE slug = ?'
        : 'UPDATE articles SET dislikes = dislikes + 1 WHERE slug = ?';
    await client.execute({ sql, args: [slug] });

    const r = await client.execute({
      sql: 'SELECT likes, dislikes FROM articles WHERE slug = ? LIMIT 1',
      args: [slug],
    });
